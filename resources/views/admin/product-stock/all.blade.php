    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Make Product Stock Information</h4>
                    </div>
            ...
            <div class="card-body">
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                    ...
                    <tr>
                        ...
                        <th>Customer delivered Quantity</th>
                   ...
                   <tbody>
                     @foreach( $products as $product)
                        <tr>
                            ...
                            <td>{{ $product->madeProducts->sum('quantity') }}</td>
                            <td>{{ $product->customerBuyProduct->sum('quantity') }}</td>
                            <td>{{ $product->madeProducts->sum('quantity') - $product->customerBuyProduct->sum('quantity') }}</td>
                            {{-- <td>{{ \App\Models\CoustomerBuyProduct::where('recipe_product_id', $product->id)->sum('quantity') }}</td> --}}
                            {{-- <td>{{ $data->quantity - $data->deliveredQuantity()}}</td> --}}
                    @endforeach
                    </tbody>
                </table>
            </div>
