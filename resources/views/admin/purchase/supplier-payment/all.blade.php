    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Supplier Payment  Information</h4>
                    </div>
                    <div class="col-md-4 text-right">
                        <a href="{{url('dashboard/material/supplier-payment/add')}}" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-plus-circle label-icon"></i>Add supplier-payment</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <div class="col-md-7">
                        @if(Session::has('success'))
                          <div class="alert alert-success alertsuccess" role="alert">
                             <strong>Success! Successfully Supplier payment paid Information</strong> {{Session::get('success')}}
                          </div>
                        @endif
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                    ...
                    <tbody>
                      ...
                      <tr>
                          ...
                          <td><strong class="text-danger">{{ ($data->previous_due_amount + $data->totalPrice())-$data->totalPayablePrice() }}</strong></td>
                          <td>{{ ($data->previous_due_amount + $data->totalPrice())-$data->totalPayablePrice() }}</td>
                          <td>{{ $data->totalPayablePrice() }}</td>
                          <td>
                              <a href="{{url('dashboard/material/supplier-payment/add/'.$data->supplier_slug)}}" title="Make payment"><i class="fas fa-dollar-sign edit_icon"></i></a>
