    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Material Stock Information</h4>
                    </div>
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                   ...
                   <tbody>
                     @foreach( $materials as $data)
                        <tr>
                            <td><a href='{{ url("dashboard/material/chalan/stock/$data->material_slug") }}'>{{ $data->material_name ?: ''}}</a></td>
                            <td>{{ $data->quantity() ?: 0 }}</td>
                            <td>{{ $data->materialDamageQuantity() ?: 0 }}</td>
                            <td>{{ $data->quantity() - ($data->materialDamageQuantity() + $data->unusedQuantity())   }}</td>
                            <td>{{ $data->unusedQuantity() ?: 0 }}</td>
                            {{--<td>
                                @php( $val = 0)
                            ...
                            <td>
                                <a href='{{ url("dashboard/material/chalan/stock/$data->material_slug") }}' title="view"><i class="fas fa-plus-square view_icon"></i></a>
