    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> <span class="text-info"></span> Material Usage Information</h4>
                    </div>
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                   ...
                   <tbody>
                     @foreach ($materials as $material)
                       <tr>
                         <td class="h4 text-uppercase text-info">{{ $material->material_name }}</td>
                         <td></td>
                         <td></td>
                         <td></td>
                         <td></td>
                       ...
                       @foreach( $material->materialPurchases as $data)
                        <tr>
                            ...
                            <td>{{ $data->ChanalDamagesQuantity() }}</td>
                            <td>{{ $data->totalmakeChalanRecipe()  }}</td>

                            <td>{{ $data->mp_quantity - ($data->ChanalDamagesQuantity() + $data->totalmakeChalanRecipe()) }} </td>
                            {{--<td>
                                @php( $val = 0)
                            ...
                            {{--<td>
                                <a href='{{url("dashboard/material/$data->material_slug/purchases-info")}}' title="view"><i class="fas fa-plus-square view_icon"></i></a>
