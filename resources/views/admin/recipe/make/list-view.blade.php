    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-5">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> Make Recipe Product
                            Information </h4>
                    </div>
                    <div class="col-md-7 text-right">
                        ...
                        <a href="{{url('dashboard/recipe-make-list')}}"
                           class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i
                                class="fas fa-th label-icon"></i>All Make Recipe List</a>
            ...
            <div class="card-body">
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                    ...
                    <tbody>
                      @foreach($all as $data)
                      <tr>
                          <td>{{ $data->chalan_name ?: '' }}</td>
                          {{-- <td>{{ $data->materials->material_name ?: '' }}</td> --}}
                          <td>{{ ($data->material_id == 9999998215) ? "Water" : $data->materials->material_name }}</td>
                          <td>{{ $data->quantity ?: '' }}</td>
                          <td>{{ $data->date ?: '' }}</td>
                          <td>
                             {{-- <a href="{{url('dashboard/material/purchase/view/')}}" title="view"><i class="fas fa-plus-square view_icon"></i></a>
                              <a href="{{url('dashboard/material/purchase/edit/')}}" title="edit"><i class="fas fa-pen-square edit_icon"></i></a>
                             --}} 
...
<div id="softDelModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="{{url('dashboard/recipe-make-list/delete')}}">
      @csrf
