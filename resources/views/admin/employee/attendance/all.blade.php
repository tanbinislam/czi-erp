    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-6">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Attendance Information</h4>
                    </div>
                    <div class="col-md-6 text-right">
                        <a href="{{url('dashboard/attendance/out-time')}}" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-plus-circle label-icon"></i>Update out-time</a>
                        <a href="{{url('dashboard/attendance/create')}}" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-plus-circle label-icon"></i>Add Attendance</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                    ...
                    <tbody>
                      @foreach($all as $data)
                      <tr>
                          <td class="{{ $data->is_holiday ? "text-danger" : '' }}" {{ $data->is_holiday ? 'title=Holiday!' : '' }}>{{date('d-F-Y', strtotime($data->date))}}</td>
                          ...
                          <td>
                               ...
                               <a href="#" title="delete" id="delete" data-toggle="modal" data-target="#softDelModal" data-id="{{$data->id}}"><i class="fas fa-trash delete_icon"></i></a>
                          </td>
                      </tr>
                      @endforeach
                    </tbody>
...
<div id="softDelModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="{{url('dashboard/attendance/soft-delete')}}">
      @csrf
