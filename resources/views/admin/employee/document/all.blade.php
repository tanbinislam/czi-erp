...
<div class="card">
    <div class="card-header card_header">
        <div class="row">
            <div class="col-md-8">
                <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> Document Information</h4>
            </div>
            <div class="col-md-4 text-right">
                <a href="#" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn" id="addInfoModal" title="add" data-toggle="modal" data-target="#addModal"><i class="fas fa-plus-circle label-icon"></i>Add Document</a>
            </div>
        </div>
    </div>
    <div class="card-body">
      <div class="row">
        <div class="col-md-12">
          <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
              ...
              <tbody>
                @foreach($emp->document as $data)
                <tr>
                    <td>{{$data->title}}</td>
                    <td>{{$data->sub_title}}</td>
                    <td><img height="50" src="{{asset('uploads/document/'.$data->image)}}"></td>
                    <td>
                      <div class="btn-group">
                          ...
                          <div class="dropdown-menu">
                              <a class="dropdown-item" href="{{url('dashboard/employee/'.$emp->employee_code.'/document/view/'.$data->id)}}">View</a>
                              <a class="dropdown-item" href="{{url('dashboard/employee/'.$emp->employee_code.'/document/edit/'.$data->id)}}">Edit</a>
