...
@section('content')
    <div class="row">
        <div class="col-12">
            <form method="post" action='{{url("dashboard/payroll/salary_setup/$salarySetup->id")}}' enctype="multipart/form-data">
                @csrf
                <div class="card">
                    <div class="card-header card_header">
                        <div class="row">
                            <div class="col-md-8">
                                <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> Update Salary Salary
                                </h4>
                            </div>
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-sm-7">
                                <select id="employeeId" class="form-control form_control" name="employee_id">
                                    ...
                                    @foreach( $employees as $employee)
                                        <option
                                            value="{{$employee->employee_id}}" {{ ($salarySetup->employee_id == $employee->employee_id) ? 'selected' : ''}}>{{ $employee->employee_name ?: '' }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('salary_type') ? ' has-error' : '' }}">
                            <label class="col-sm-3 col-form-label col_form_label">Salary Type <span
                                    class="req_star">*</span>:</label>
                            <div class="col-sm-7">
                                <input type="text" id="salaryType" readonly class="form-control form_control"
                                       name="salary_type"
                                       value="{{ isset($salarySetup->employees) ? isset($salarySetup->employees->official) ? $salarySetup->employees->official->salary_type : '' : '' }}">

                            </div>
                        </div>
                        <div class="form-group row mb-3 {{ $errors->has('is_percentage') ? ' has-error' : '' }}">
                            <label class="col-sm-3 col-form-label col_form_label">Percentage <span
                                    class="req_star">*</span>:</label>
                            <div class="col-sm-7">
                                <input value="1" style="margin-top: 13px;" class="checkbox" type="checkbox" name="is_percentage"
                                       id="ispercentage" {{ isset($salarySetup->is_percentage) ? 'checked' : '' }}>
                                            ...
                                            <table id="add">
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input style="margin-bottom: 12px" type="number"
                                                               ...
                                                               autocomplete="off" value="{{ isset($salarySetup->employees) ? isset($salarySetup->employees->official) ? $salarySetup->employees->official->salary : '' : '' }}"></td>
                                                </tr>
                                                @foreach( $salaryTypes as $salaryType)
                                                    <tr>
                                                        <th style="padding-right: 18px;"
                                                            class="padten">{{ $salaryType->name }}
                                                            <span class="percent d-none">(%)</span></th>
                                                        <td><input style="margin-bottom: 12px" type="number"
                                                                   id="amount_{{$salaryType->id}}"
                                                                   class="form-control amount"
                                                                   value=""
                                                                   data-btype={{$salaryType->type}}></td>
                                                    </tr>
                                                @endforeach
...
@push('scripts')
    {{-- <script type="text/javascript">
        $(document).ready(function () {
            ...
            const upSalary = $("#basicSalary").val();

            $("#grossSalary").val('{{ $salarySetup->gross_salary }}');
            ...
            $('.amount').keyup(function () {
                ...
                var total = parseInt(upSalary) + parseInt(sum);
                console.log(typeof total);
                $("#grossSalary").val(upSalary + sum);
            });
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            let employeeId = '{{ $salarySetup->employee_id }}'
                $("#basicSalary").val(null);
                $("#salaryType").val(null);
                $("#grossSalary").val(null);
                $(".amount").val(null);
