...
@section('content')
@php
    $all=App\Models\Customer::where('customer_status',0)->orderBy('customer_id','DESC')->get();
@endphp
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> Customer Trash List</h4>
                    </div>
                        ...
                        <td>{{$data->customer_name}}</td>
                          ...
                          <td>
                            @if($data->customer_photo!='')
                              <img class="img-thumbnail img50" src="{{asset('uploads/customer/'.$data->customer_photo)}}"/>
                            @else
                              <img class="img-thumbnail img50" src="{{asset('uploads/avatar/avatar-black.png')}}"/>
                            @endif
                          </td>
                          <td>
                              <a href="#" id="restore" title="restore" data-toggle="modal" data-target="#restoreModal" data-id="{{$data->customer_id}}"><i class="fas fa-recycle restore_icon"></i></a>
                              <a href="#" id="delete" title="delete" data-toggle="modal" data-target="#deleteModal" data-id="{{$data->customer_id}}"><i class="fas fa-trash delete_icon"></i></a>
                          </td>
                      </tr>
                      @endforeach
                    </tbody>
...
<div id="restoreModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="{{url('dashboard/customer/restore')}}">
      @csrf
