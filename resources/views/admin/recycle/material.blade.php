...
@section('content')
@php
    $all=App\Models\Material::where('material_status',0)->orderBy('material_id','DESC')->get();
@endphp
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> Material Trash List</h4>
                    </div>
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td>{{$data->material_name}}</td>
                          <td>{{$data->material_remarks}}</td>
                          <td>
                            @if($data->material_photo!='')
                              <img class="img-thumbnail img50" src="{{asset('uploads/material/'.$data->material_photo)}}"/>
                            @else
                              <img class="img-thumbnail img50" src="{{asset('uploads/avatar/avatar-black.png')}}"/>
                            @endif
                          </td>
                          <td>
                              <a href="#" id="restore" title="restore" data-toggle="modal" data-target="#restoreModal" data-id="{{$data->material_id}}"><i class="fas fa-recycle restore_icon"></i></a>
                              <a href="#" id="delete" title="delete" data-toggle="modal" data-target="#deleteModal" data-id="{{$data->material_id}}"><i class="fas fa-trash delete_icon"></i></a>
                          </td>
                      </tr>
                      @endforeach
                    </tbody>
...
<div id="restoreModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="{{url('dashboard/material/restore')}}">
      @csrf
...
<div id="deleteModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="{{url('dashboard/material/delete')}}">
      @csrf
      <div class="modal-content">
        <div class="modal-header modal_header">
            <h5 class="modal-title mt-0" id="myModalLabel"><i class="fab fa-gg-circle"></i> Confirm Message</h5>
